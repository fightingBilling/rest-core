...
import com.somnus.restful.core.client.proxy.ResourceJsonInvocationHandler;
 ...
 * @version 1.0
 */
public class RESTfulJsonClientFactory {
	...
	private static <C> C proxyClient(Class<C> interfaceType, Parameter param){
        ...
        if(proxy==null){
            proxy = Proxy.newProxyInstance(
                    RESTfulJsonClientFactory.class.getClassLoader(),
                    interfaces,
                    new ResourceJsonInvocationHandler(param)
            );
